  ...
  list.forEach((href) => {
    _get(href).then((resp) => {
      ...
      const title = htmlDom('head')
        ...
        .attr('content');
      if (title) {
        getSimplePDF(href, filenamify(title));
      } else {
        ERROR_MESSAGE.list.push({
          href,
          title: '标题没有获取到，请查验是否正常',
        });
...
const getSimplePDF = (url, name) => {
  const dirRoot = path.join(__dirname, '../docs/mppdf/');
  isExistDist(dirRoot);
  percollate.configure();
  percollate.pdf([url], {
    output: dirRoot + name + '.pdf',
    css: '@page { size: A6 landscape } html { font-size: 12pt; font-family: Noto Sans} ', // 生成pdf的大小和字体,
  });
};
